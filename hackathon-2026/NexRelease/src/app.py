...
def get_all_setup_users():
    """Return list of all usernames who have completed setup (have Jira + Slack credentials)."""
    profiles = load_profiles()
    return [
        uname for uname, profile in profiles.items()
        if profile.get("jira_api_token") and profile.get("slack_token")
    ]
...
function showSecurityAlert(n, idx) {{
  document.getElementById('sec-modal-msg').textContent =
    `PR #${{n.pr_number}} "${{n.pr_title}}" — @${{n.author}} is not on the whitelist.`;
  document.getElementById('sec-modal-details').innerHTML =
    `PR: #${{n.pr_number}}<br>Author: @${{n.author}}<br>Time: ${{n.time}}`;
  document.getElementById('sec-approve-btn').onclick = async () => {{
    dismissAlert();
...
def webhook():
    ...
    # ── KEY FIX: get ALL users who have completed setup ──
    all_users = get_all_setup_users()
    if not all_users:
        return jsonify({"status": "no users configured"})
        ...
        try:
            ...
            if security["status"] == "unauthorized":
                ...
                for uname in all_users:
                    add_notification(
                        username=uname, pr_number=pr_number,
                        pr_title=pr_data["title"], author=pr_data["author"],
                        jira_url="", slack_posted=False, meeting="",
                        security=security, pr_url=pr_data["pr_url"],
                        summary="⚠️ Unauthorized contributor opened a PR.",
    ...
    if action == "closed" and is_merged:
        ...
        try:
            ...
            if security["status"] == "unauthorized":
                # Notify ALL setup users about the unauthorized merge
                for uname in all_users:
                    add_notification(
                        username=uname, pr_number=pr_number,
                        pr_title=pr_data["title"], author=pr_data["author"],
                ...
                return jsonify({"status": "security alert — all users notified"})

            summary = summarize_pr(pr_data)

            # ── Create Jira ticket + post Slack for EACH user with their OWN credentials ──
            for uname in all_users:
                jira_creds  = get_user_jira(uname)
                slack_creds = get_user_slack(uname)

                # Create ticket in this user's Jira
                ticket = create_jira_ticket(
                    title=summary["jira_title"], description=summary["summary"],
                    checklist=summary["checklist"], risks=summary["risks"],
                    pr_id=pr_number, **jira_creds
...
*PR:* {pr_data['pr_url']}""".strip()
                ...
                # Save notification to this user's feed
                add_notification(
                    username=uname, pr_number=pr_number,
                    pr_title=pr_data["title"], author=pr_data["author"],
                    jira_url=ticket.get("url", ""), slack_posted=True,
            ...
            return jsonify({"status": "agent triggered", "pr": pr_number, "notified_users": all_users})
        except Exception as e:
            return jsonify({"status": "error", "message": str(e)})
